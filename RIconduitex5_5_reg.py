...
# con la fragmentación regularizada (rampa tanh de ancho eps).
# NO reemplaza al original. Ver fragmentacion_reg.py.
#
...
from fvrel import *
from fragmentacion_reg import s_frag, fuerzas_mezcla, EPS_FRAG
...
def _mezclar_fragmentacion(phi, rho_g, um, ug, Fmw_v, Fgw_v, Fmg_v):
    ...
    s = float(s_frag(phi, phicrit, eps_frag))
    ra = 1e-3
    cd = 0.8
    Fmw_f = 0.0
    Fgw_f = 0.01 * rho_g * np.abs(ug) * ug / (4.0 * wr)
    Fmg_f = 3 * cd * rho_g * np.abs(ug - um) * (ug - um) * phi * (1.0 - phi) / (8.0 * ra)
    Fmw, Fgw, Fmg = fuerzas_mezcla(s, Fmw_v, Fgw_v, Fmg_v, Fmw_f, Fgw_f, Fmg_f)
    return Fmw, Fgw, Fmg, s
    ...
    if n_eq in [1, 2, 3]:
        ...
        if n_eq == 3:
            if rb<lim*wr and y[1]<0.5:
                dNdt=-(y[2]**(2/3))*((1/(1-y[1]))**(1/3))*((1/9)*(rho_m-rho_g)*9.81/visc)*((3*y[1]/(4*np.pi))**(2/3))*(F1*F1 - F2*F2)*(1/(1-((F1+F2)*((3*y[1]*(np.pi/(6*phicrit))/(4*np.pi))**(1/3)))))*Fc*(1-y[1]/phicrit)*(wr-rb)/wr
            else: 
                dNdt=0
        else:
...
#Función principal del programa (laboratorio: eps es el ancho de la rampa)
def RIconduitex5_5_reg_f(radius,Pressure, wt, Temperature, content_crystal, eps=EPS_FRAG):
...
RIconduitex5_5_f = RIconduitex5_5_reg_f