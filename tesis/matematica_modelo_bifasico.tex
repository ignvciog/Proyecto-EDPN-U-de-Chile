...
(C_1,\beta)
=\begin{cases}
(4.11\times10^{-6},\;1/2) & \text{silíceos},\\
(6.80\times10^{-8},\;0.7) & \text{basaltos (Villarrica)}.
\end{cases}
\end{equation}
...
\end{equation}
$v_{\mathrm{in}}$ \textbf{no} es un dato: es el parámetro de tiro. El
DAE es el mismo para erupciones explosivas y efusivas; cambian solo
las condiciones de salida. Hay dos códigos, no dos modelos:

\paragraph{Explosivo (\texttt{RIconduitex5\_5.py}).}
Yoshida \& Koyaguchi (1999) / Kozono \& Koyaguchi (2009):
\begin{equation}
\label{eq:bc-top}
P(0)=P_{\mathrm{atm}}
\quad\text{o}\quad
...
\end{equation}
con $\phi(0)\ge\phi_{\mathrm{crit}}$ (fragmentó) y
$z_{\mathrm{exit}}\ge-5\,\mathrm{m}$. $c_s$ es la velocidad del
sonido isoterma del gas.

\paragraph{Efusivo (\texttt{RIconduitef5\_5.py}).}
El magma llega a la superficie \emph{sin} fragmentar:
\begin{equation}
\label{eq:bc-ef}
P(0)=P_{\mathrm{atm}},
\qquad
\phi(0)\le\phi_{\mathrm{crit}},
\qquad
z_{\mathrm{exit}}\in(-2,0].
\end{equation}
El $v_{\mathrm{in}}$ inicial de la búsqueda es $0.1\,\mathrm{m\,s^{-1}}$
(frente a $10\,\mathrm{m\,s^{-1}}$ en el explosivo).

Para un mismo $\theta$ puede existir $q_{\mathrm{ex}}^\ast$,
$q_{\mathrm{ef}}^\ast$, las dos, o ninguna
(Castruccio \emph{et al.}, 2025). El operador directo del paper es el
par
\begin{equation}
\label{eq:F}
\mathcal{F}(\theta)
...
\end{equation}
Invertir solo un $\MER$ sin el estilo deja las dos ramas confundidas.
...
\bibitem{castruccio2025} Castruccio, A., Rebolledo, A.\ \& Gómez, I.
  (2025). \emph{The influence of melt composition, temperature,
  crystallinity and water content on eruptive style and eruption rate:
  insights from a conduit model of magma ascent}. JGR Solid Earth 130,
  e2024JB030599.
