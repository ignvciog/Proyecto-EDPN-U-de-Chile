...
\begin{frame}{El prototipo: un laboratorio matemático}
Para no esperar a la unificación de los solvers, construí un \alert{modelo
reducido} (\texttt{tesis/prototipo/}) que comparte las constitutivas del
repositorio (\texttt{density.py}, \texttt{viscosity.py}, \texttt{fvrel.py}) y es:
\medskip
\begin{columns}[T]
...
\begin{itemize}\small
  ...
  \item[(iv)] consistente con el modelo completo en el régimen viscoso
    pre-fragmentación.
\end{itemize}
\end{column}
\end{columns}
...
% ---------------------------------------------------------------------------
\begin{frame}{Síntesis de los tres resultados}
\begin{columns}[T]
\begin{column}{0.32\textwidth}
\begin{block}{1. Degeneración}
\small El $\MER$ estacionario ve $R(\cdot)$ sólo a través de \alert{un escalar}.
La preimagen de un dato es una hipersuperficie de codimensión 1 en un espacio de
funciones.\smallskip
...
\small $+317\pc$ de diferencia de volumen con el mismo $\MER$. El 90 \% de la
caída de presión está en el kilómetro más somero: \alert{el conducto profundo es
invisible}.
\end{block}
\end{column}
\begin{column}{0.32\textwidth}
\begin{block}{3. El tiempo ayuda}
\small Pero no lo resuelve todo: identifica $R$, sólo acota $\Delta P$ por
arriba. \alert{Cuánto aporta cada dimensión se puede medir.}
\end{block}
\end{column}
\end{columns}

\vspace{1.2em}
\mensaje{Los tres son reproducibles hoy: \texttt{tesis/prototipo/exp1..exp3}.
...
\begin{description}
  ...
  \item[\textbf{Cap.\ 3 — Identificabilidad} ] Fisher y perfiles de verosimilitud
    para todos los parámetros. Comparación sistemática: estacionario 1D
    vs.\ transiente 1D vs.\ estacionario 2D.
